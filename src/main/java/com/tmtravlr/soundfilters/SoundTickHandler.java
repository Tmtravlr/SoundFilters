...
/**
 * This class handles the tick events and updates the filter parameters
 * for each source playing.
 * 
 * @author Rebeca Rey
 * @Date 2014
 */
public class SoundTickHandler
{
	...
	public static Comparator<ComparablePosition> CPcomparator = new Comparator<ComparablePosition>()
		...
		public int compare(SoundTickHandler.ComparablePosition first, SoundTickHandler.ComparablePosition second)
		{
			 ...
			 * that make comparing it with another ComparablePositions very fast. Used for
			 * fast-access sets/maps.
			 * 
			 * @author Rebeca Rey
			 * @Date 2014
			 */
			...
			/**
			 * Represents two values for use in the occlusion: the sound source and occlusion amount.
			 * Also has a timeout variables, which the source should be constantly setting to 10. When
			 * the source stops setting it to 10, the sound is assumed to have finished playing.
			 * 
			 * @author Rebeca Rey
			 * @Date 2014
			 */
				...
				else
				{
					...
					{	
							...
							if (sourceAndAmount != null) {
								if (sourceAndAmount.source != null && sourceAndAmount.source.position != null && sourceAndAmount.source.playing() && !sourceAndAmount.source.stopped() && sourceAndAmount.timeout > 0)
								{
									...
									if (this.mc != null && this.mc.theWorld != null && this.mc.thePlayer != null)
									{
										...
										sourceAndAmount.amount = (sourceAndAmount.amount*3 + getSoundOcclusion(this.mc.theWorld, new Vec3d((double)sourceAndAmount.source.position.x, (double)sourceAndAmount.source.position.y, (double)sourceAndAmount.source.position.z), roomSize)) / 4.0;
									}
									else
									{
								...
								{
									//Remove any sources that have "timed out" (have stopped setting the timeout 
						...
						for (ComparablePosition positionToRemove : toRemove) {
							if(SoundFiltersMod.SUPER_DUPER_DEBUG) System.out.println("[Sound Filters] Removing " + positionToRemove + ", " + positionToRemove.hashCode() + ", " + profileTickCountdown);
					...
					if (this.mc != null && this.mc.theWorld != null && this.mc.thePlayer != null && SoundFiltersMod.doReverb)
					{
						...
						if (profileTickCountdown <= 0)
						{
							...
							//Flood fill through the area the player is in, with maximum size of 
							...
							for (i = 0; i < SoundFiltersMod.profileSize && !toVisit.isEmpty(); i++)
							{
								...
								//Check for valid neighbours in 6 directions (valid means a non-solid block that hasn't been visited)
							...
							for(IBlockState s : blocksFound)
							{
								...
								{
									if (s.getMaterial() != Material.CACTUS && s.getMaterial() != Material.CAKE && s.getMaterial() != Material.CLOTH && s.getMaterial() != Material.CORAL && s.getMaterial() != Material.GRASS && s.getMaterial() != Material.LEAVES && s.getMaterial() != Material.CARPET && s.getMaterial() != Material.PLANTS && s.getMaterial() != Material.GOURD && s.getMaterial() != Material.SNOW && s.getMaterial() != Material.SPONGE && s.getMaterial() != Material.VINE && s.getMaterial() != Material.WEB)
									{
										//Generic materials that don't fall into either catagory (wood, dirt, etc.)
										++midReverb;
									}
									else
									{
										//Reverb-absorbing materials (like wool and plants)
										++lowReverb;
									}
								}
								else
								{
									//Materials that relfect sound (smooth and solid like stone, glass, and ice)
									++highReverb;
								}
							...
							if(SoundFiltersMod.doSkyChecks && roomSize == SoundFiltersMod.profileSize) {
								/*Check if you and blocks around you can see the sky in a pattern like so:
						        ...
						        With distances of random from 5 to 10, and also above 5 for the B's
								*/
								...
								if(onlySkyAboveBlock(mc.theWorld, x, y, z)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x+rand.nextInt(5)+5, y, z)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x-rand.nextInt(5)+5, y, z)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x, y, z+rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x, y, z-rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x+rand.nextInt(5)+5, y, z+rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x-rand.nextInt(5)+5, y, z+rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x+rand.nextInt(5)+5, y, z-rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x-rand.nextInt(5)+5, y, z-rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x+rand.nextInt(5)+5, y+5, z)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x-rand.nextInt(5)+5, y+5, z)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x, y+5, z+rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x, y+5, z-rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x+rand.nextInt(5)+5, y+5, z+rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x-rand.nextInt(5)+5, y+5, z+rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x+rand.nextInt(5)+5, y+5, z-rand.nextInt(5)+5)) skyFactor++;
								if(onlySkyAboveBlock(mc.theWorld, x-rand.nextInt(5)+5, y+5, z-rand.nextInt(5)+5)) skyFactor++;
							...
							if(SoundFiltersMod.SUPER_DUPER_DEBUG) System.out.println("[Sound Filters] Reverb Profile - Room Size: " + roomSize + ", Looked at: " + i + ", Sky Factor: " + skyFactor + ", High, Mid, and Low Reverb: (" + highReverb + ", " + midReverb + ", " + lowReverb + ")");
							...
							SoundFiltersMod.reverbFilter.decayTime = 8.0F * decayFactor * roomFactor * skyFactor;
							...
							SoundFiltersMod.reverbFilter.reflectionsGain = 0.05F + (0.05F * roomFactor);
							SoundFiltersMod.reverbFilter.reflectionsDelay = 0.025F * roomFactor;
							SoundFiltersMod.reverbFilter.lateReverbGain = 1.26F + (0.1F * roomFactor);
			...
			/**
			 * Ray traces through the world from sound to listener, and returns the amount of occlusion
			 * the sound should have. Note that it ignores the block which the sound plays from inside 
			 * (so sounds playing from inside blocks don't sound occluded).
			 * 
			 * @param world The world
			 * @param sound The position the sound plays from
			 * @param listener The listener's position (the player)
			 * @return A double representing the amount of occlusion to apply to the sound
			 */
